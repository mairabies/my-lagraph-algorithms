...
//
//   Part 1 — All-pairs:
//     LAGraph_NumberOfWalks (src=NULL) vs LAGraph_MultiSourceBFS
//     Both produce an n x (n or nsrc) matrix. NumberOfWalks uses binary
//     exponentiation (O(log k) mxm calls); MultiSourceBFS uses k mxm calls.
//
//   Part 2 — Single-source:
...
// (1-based) per row in column 0, matching the GAP benchmark format used by
// bfs_demo.  If omitted, the first N_DEFAULT_SOURCES node indices are used.
...
// Default walk length and number of sources when no source file is given
#define DEFAULT_K         4
#define N_DEFAULT_SOURCES 64
// Max source trials for single-source timing. Recomputing A^k per trial is
// expensive on large graphs — keep this small for practical benchmark runs.
#define MAX_TRIALS        8
...
int main (int argc, char **argv)
{
    ...
    GrB_Vector src_indicator  = NULL ;  // indicator (1 at src) for single-source walks
    GrB_Vector multisrc_vector = NULL ; // source list for MultiSourceBFS
    GrB_Matrix SourceNodes = NULL ;

    double *t_walks_all = NULL ;  // all-pairs NumberOfWalks time per thread count
    double *t_msbfs     = NULL ;  // MultiSourceBFS time per thread count
    double *t_walks_src = NULL ;  // avg single-source NumberOfWalks per thread count
    ...
    // This avoids passing k as argv[3], which would confuse readproblem
    // (readproblem treats argv[2] as sources file, argv beyond that is ignored
    // but passing extra args risks opening "4" as a filename).
    // Usage: WALK_K=6 ./walks_demo graph.mtx [sources.mtx]
    //--------------------------------------------------------------------------
    ...
    //--------------------------------------------------------------------------
    // thread list setup (copied from bfs_demo.c)
    //--------------------------------------------------------------------------
    ...
    //--------------------------------------------------------------------------
    // load graph (INT64 so walk counts are computed in the right type)
    // and optional source nodes file
    //--------------------------------------------------------------------------

    char *matrix_name = (argc > 1) ? argv [1] : "stdin" ;
    LAGRAPH_TRY (readproblem (&G, &SourceNodes,
        false,      // make_symmetric: keep as-is (walks work on directed graphs)
    ...
    if (SourceNodes == NULL)
    {
        ...
        {
            // store 1-based indices to match GAP format (readproblem decrements)
    ...
    // cap trials to keep single-source loop tractable (A^k recomputed per trial)
    ...
    //--------------------------------------------------------------------------
    // warmup (uncached runs — not timed for results)
    //--------------------------------------------------------------------------
    ...
    // warmup: single-source BFS and NumberOfWalks (first source)
    {
        int64_t src ;
    ...
    //==========================================================================
    // timed benchmark across thread counts
    //==========================================================================
    ...
    for (int tt = 1 ; tt <= nt ; tt++)
    {
        ...
        // NumberOfWalks — one run produces the full A^k matrix
        {
        ...
        // MultiSourceBFS — one run over all ntrials source nodes
        {
            double t_run = LAGraph_WallClockTime () ;
        ...
        //----------------------------------------------------------------------
        // Part 2: Single-source NumberOfWalks vs BFS, averaged over all sources
        //----------------------------------------------------------------------
        ...
        for (GrB_Index trial = 0 ; trial < ntrials ; trial++)
        {
            ...
            // single-source NumberOfWalks (indicator vector: 1 at src)
            ...
            GRB_TRY (GrB_free (&C)) ;
            // clear the vector for the next trial
        ...
        //----------------------------------------------------------------------
        // summary for this thread count (also to stderr for script parsing)
        //----------------------------------------------------------------------
